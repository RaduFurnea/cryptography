...
public class MainWindowController {
	...
	/*
	 * public void showDecryptStage() throws IOException{ FXMLLoader loader = new
	 * FXMLLoader(); loader.setLocation(getClass().getResource(
	 * "../../resources/DecryptWindow.fxml")); decryptLayout = loader.load();
	 * decryptStage = new Stage();
	 * decryptStage.setTitle("Select Paths for Decrypted File");
	 * decryptStage.initModality(Modality.WINDOW_MODAL); Scene scene = new
	 * Scene(decryptLayout,285,153);
	 * 
	 * decryptStage.setScene(scene); decryptStage.show(); }
	 */
	...
	private void verifyAdded() {
		try {
			...
			if (!mainFile.getPath().equals(null) && !publicF.getPath().equals(null)
					&& !signatureF.getPath().equals(null))
		...
		try {
			...
			if (event.getSource() == sign) {
				...
				if (privateF == null || mainFile == null || signatureF == null) {
					...
					new Messages(UiMain.password).sign(mainFile.getPath(), privateF.getPath(), signatureF.getPath());
				}
			}
			if (event.getSource() == verify) {
				if (publicF.getPath().equals(null) || mainFile.getPath().equals(null)) {
					...
					if (new Messages(UiMain.password).verify(mainFile.getPath(), publicF.getPath(), signatureF.getPath())) {
